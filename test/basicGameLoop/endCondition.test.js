...
describe("Basic Game Loop Test: End Game", function (){
    ...
    it("Tests game end state", function() {
        ...
        const redBee = new queenBee(firstPlayer,[6,4])
        red.queenBee = redBee
        board.addToBoard(redBee)

        const moveAnt = new ant(secondPlayer,[2,4])
        board.addToBoard(moveAnt)
        const staticAnt2 = new ant(secondPlayer,[4,5])
        board.addToBoard(staticAnt2)
        const staticAnt3 = new ant(secondPlayer,[6,6])
        board.addToBoard(staticAnt3)
        const staticAnt4 = new ant(secondPlayer,[8,5])
        board.addToBoard(staticAnt4)
        const staticAnt5 = new ant(secondPlayer,[8,3])
        board.addToBoard(staticAnt5)
        const staticAnt6 = new ant(secondPlayer,[6,2])
