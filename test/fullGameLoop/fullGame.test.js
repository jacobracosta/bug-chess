...
import loopTestDrawJson from "../testData/fullGame.draw.json" assert { type: 'json' };
...
describe("Basic Game Loop Test: Full Game Loop", function (){
    ...
    it("Tests Full Loop: Success, Draw", function (){ 
        const [gameOverMessage, errors] = gameLoopTestHandle(loopTestDrawJson["commands"])
        expect(gameOverMessage).to.eq("Game Over! Draw.")
        expect(errors).to.be.an("array").that.is.empty
    })
